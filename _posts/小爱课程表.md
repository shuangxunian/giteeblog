...
2. 说：打开课程表。
3. 点最右边标签，找到教务网站导入（需要连接校园网），点进去。
4. 输入账号密码，信息查询，学生课表查询。
5. 点击下方一键导入。
6. 导入成功后，选择当前周数，改成对应周数；选择学校，选择我编写的版本即可。
...
3. 3. 在Chrome打开一个新的Tab，打开、登陆自己的教务系统，进入课程页面。
...
**注意：**代码中无需引用任何模块，不要带有“require”这样的字符
